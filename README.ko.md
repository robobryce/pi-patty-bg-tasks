...
**Claude Code의 백그라운드 작업 경험을 Pi로.** 장시간 실행 명령이 에이전트를 차단하지 않습니다 — 120초 후 자동 백그라운드, Ctrl+Shift+B로 수동 백그라운드, 출력 캡처, 정체 감지, 통합 작업 관리자.
...
Pi v0.37+ 필요. 외부 의존성 없음 — 백그라운드 잡은 파일 디스크립터 출력 캡처를 사용하는 Node.js 자식 프로세스로 직접 실행됩니다.

## v1.0 호환성 변경 사항

- **tmux 의존성 제거.** 백그라운드 잡은 이제 파일 디스크립터 출력 캡처를 사용하는 Node.js `child_process.spawn` 프로세스로 직접 실행됩니다 (Claude Code 동일). tmux는 더 이상 사용되거나 필요하지 않습니다 — 설치할 것이 없습니다.
- **기본 자동 백그라운드 타임아웃이 이제 120초입니다** (이전 15초), Claude Code와 동일. 재정의하려면 명시적 `timeout`을 전달하세요.

## 왜 pi-patty-bg-tasks인가

**세션 차단 없음.** 개발 서버, 테스트 스위트, 빌드 — 120초 이상 실행되는 모든 명령이 자동으로 백그라운드됩니다. 에이전트는 알림을 받고 즉시 다음 작업을 계속합니다. 언제든 수동으로 명령을 백그라운드할 수도 있습니다.
...
```
# 에이전트가 긴 명령 실행 — 120초 후 자동 백그라운드
bash({ command: "npm run build" })

# run_in_background로 처음부터 백그라운드로 시작
...
내장 bash 도구를 확장합니다. 명령은 정상적으로 실행되지만, 120초를 초과하면 자동으로 백그라운드되고 에이전트에게 `job_decide`를 통해 결정(유지, 종료, 출력 확인)을 요청합니다.

| 파라미터 | 설명 |
|---------|------|
| `command` | 실행할 셸 명령 |
| `timeout` | 커스텀 타임아웃 (초 단위, 기본값: 120) |
| `run_in_background` | 포그라운드 실행과 자동 백그라운드 타이머를 건너뛰고 명령을 즉시 백그라운드로 시작 |
...
자동 백그라운드된 명령에 대한 결정. 120초 타이머가 발동하면 에이전트가 이 프롬프트를 받습니다.
...
```
명령 시작 (Node.js child_process.spawn 직접 실행)
  → 2초 이내 완료?         결과 즉시 반환
  → 120초에도 실행 중?     자동 백그라운드 → 에이전트에 job_decide 프롬프트
  → 사용자가 Ctrl+Shift+B?  즉시 백그라운드 → 에이전트 계속

백그라운드 잡 실행 중
  → 파일 디스크립터로 출력을 /tmp/pi-bg-<id>.log에 캡처
  → 정체 감지: 출력이 인터랙티브 프롬프트처럼 보이면 에이전트에 경고
  → 과대 출력 감지: 제한 초과 시 잡 종료
  → 완료 시: 에이전트에 상태 + 출력 경로 알림
```

백그라운드 잡은 stdout/stderr을 로그 파일 디스크립터에 직접 연결한 detached Node.js
자식 프로세스로 실행됩니다 — Claude Code와 동일한 패턴입니다. tmux나 외부 프로세스
관리자는 관여하지 않습니다.
...
Node.js ≥ 22, pnpm ≥ 10 필요. tmux 등 외부 의존성 없음.
